$(document).ready(function() {
    ...
    $(document).delegate('.exchange-rate-edit', 'click', function() {
        ...
        $('#edit-exchange-rate-modal').data('pair', pair);  // Сохраняем валютную пару для дальнейшего использования
    ...
    $('#edit-exchange-rate-modal .btn-primary').click(function() {
        ...
        $.ajax({
            url: `${host}/exchangeRate/${pair}?rate=${exchangeRate}`,  // Параметр rate передается в URL
            type: "PATCH",
            success: function(response) {
                // Найти строку по валютной паре и обновить курс
                const row = $(`tr[data-pair="${pair}"]`);
                row.find('td:eq(1)').text(response.rate);  // Обновить курс
                $('#edit-exchange-rate-modal').modal('hide');  // Закрыть модальное окно
            },
            error: function(jqXHR, textStatus, errorThrown) {
                const error = JSON.parse(jqXHR.responseText);
                const toast = $('#api-error-toast');
